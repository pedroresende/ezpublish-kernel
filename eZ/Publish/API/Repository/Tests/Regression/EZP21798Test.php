...
class EZP21798Test extends BaseTest
{
    ...
    public function testRoleChanges()
    {
        ...
        $articleLocation = $urlAliasService->lookup( '/News/Article-1' );

        $contentInfoarticle = $locationService->loadLocation( $articleLocation->destination )->contentInfo;

        $section = $sectionService->loadSectionByIdentifier( 'private' );
        $sectionService->assignSection( $contentInfoarticle, $section );

        $contentInfoarticle = $contentService->loadContentInfo( $contentInfoarticle->id );
        ...
        Assertion::assertTrue( $found, "Couldn't find policy with module 'content' and function 'read'" );
